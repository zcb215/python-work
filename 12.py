...
optimizer = optim.Adam(model.parameters(), lr=0.2,weight_decay=0.002)
...
for epoch in range(num_epochs):
    ...
    for batch_data, batch_labels in train_loader:
        #print("Batch data:", type(batch_data), batch_data.shape)
        #print("Batch labels:", type(batch_labels), batch_labels.shape)
        #Batch data: <class 'torch.Tensor'> torch.Size([32, 1747])
        #Batch labels: <class 'torch.Tensor'> torch.Size([32, 1])
        optimizer.zero_grad()

        # 前向传播和计算损失
        #print("new data:", type(batch_data.unsqueeze(0)), batch_data.unsqueeze(0).shape)
        #new data: <class 'torch.Tensor'> torch.Size([1, 32, 1747])
